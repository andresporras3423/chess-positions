...
namespace ConsoleApp1
{
    class Startup
    {
        static List<Cell> knightMovements = new List<Cell>{new Cell(1,2), 
            new Cell(1, -2), 
            new Cell(2, 1), 
            new Cell(2, -1), 
            new Cell(-1, 2), 
        ...
        static List<Cell> kingMovements = new List<Cell>{new Cell(1,1),
            new Cell(1, 0),
            new Cell(1, -1),
            new Cell(0, 1),
            new Cell(0, -1),
        ...
        static List<Cell> bishopMovements = new List<Cell>{new Cell(1,1),
            new Cell(1, -1),
            new Cell(-1, 1),
            new Cell(-1, -1)};

        static List<Cell> rockMovements = new List<Cell>{new Cell(0,1),
            new Cell(0, -1),
            new Cell(1, 0),
            new Cell(-1, 0)};

        static string last_movement = ",,,,";
        static bool black_king_move = false;
        static bool black_rock1_move = false;
        static bool black_rock2_move = false;
        static bool white_king_move = false;
        static bool white_rock1_move = false;
        static bool white_rock2_move = false;
        ...
        static string[,] tempCells;



        static Dictionary<string, Cell> blackPieces = new Dictionary<string, Cell>
        {
            {  "br1", new Cell(0,0) },
            {  "bn1", new Cell(0,1) },
            {  "bb1", new  Cell(0,2)},
        ...
        static Dictionary<string, Cell> whitePieces = new Dictionary<string, Cell>
        {
            {  "wp1", new Cell(6,0) },
            {  "wp2", new Cell(6,1) },
            {  "wp3", new Cell(6,2) },
        ...
        public static bool white_king_attacked(Cell king)
        {
        ...
        public static bool black_king_attacked(Cell king)
        {
        ...
        public static HashSet<string> available_black_moves()
        {
        ...
        public static HashSet<string> available_black_king_moves()
        {
            ...
            foreach (var kingMovement in kingMovements)
            {
                ...
                if (nCell == "" || Regex.Match(nCell, @"^w").Success)
                {
                    ...
                    {
                        availableMovements.Add($"bk,{king.y + kingMovement.y},{king.x + kingMovement.x}");
                    }
                }
            }
            tempCells = (string[,])cells.Clone();
            ...
            {
                availableMovements.Add($"bk,{king.y},{king.x - 2}");
            }
        ...
        public static HashSet<string> available_black_knight_moves(string piece, Cell knight)
        {
            Cell king = blackPieces["bk"];
            HashSet<string> availableMovements = new HashSet<string>();
            foreach (var knightMovement in knightMovements)
            {
                ...
                if (nCell == "" || Regex.Match(nCell, @"^w").Success)
                {
                    ...
                    {
                        availableMovements.Add($"{piece},{knight.y + knightMovement.y},{knight.x + knightMovement.x}");
                    }
                }
        ...
        public static HashSet<string> available_black_pawn_moves(string piece, Cell pawn)
        {
            Cell king = blackPieces["bk"];
            HashSet<string> availableMovements = new HashSet<string>();
            ...
            if (nCell == "")
            {
                ...
                if(!black_king_attacked(new Cell(king.y, king.x))) availableMovements.Add($"{piece},{pawn.y + 1},{pawn.x}");
            }
            if (pawn.y == 1)
            {
                ...
                if (nCell == "")
                {
                    ...
                    if (!black_king_attacked(new Cell(king.y, king.x))) availableMovements.Add($"{piece},{pawn.y + 2},{pawn.x}");
                }
            }
            ...
            if (Regex.Match(nCell, @"^w").Success)
            {
                ...
                if (!black_king_attacked(new Cell(king.y, king.x))) availableMovements.Add($"{piece},{pawn.y + 1},{pawn.x + 1}");
            }
            nCell = valid_position(pawn.y + 1, pawn.x - 1);
            if (Regex.Match(nCell, @"^w").Success)
            {
            ...
            if (last_movement == $"wp,{pawn.y+2},{pawn.x+1},{pawn.y},{pawn.x + 1}")
            {
                tempCells = (string[,])cells.Clone();
                tempCells[pawn.y, pawn.x] = "";
        ...
        public static HashSet<string> available_black_bishop_moves(string piece, Cell bishop)
        {
            Cell king = blackPieces["bk"];
            HashSet<string> availableMovements = new HashSet<string>();
            foreach (var bishopMovement in bishopMovements)
            {
                ...
                while (true)
                {
                    ...
                    if (nCell == "" || Regex.Match(nCell, @"^w").Success)
                    {
                        ...
                        if (!black_king_attacked(new Cell(king.y, king.x)))
                        {
                            availableMovements.Add($"{piece},{nPosition.y},{nPosition.x}");
                        }
                    }
                    if (nCell != "") break;
                }
        ...
        public static HashSet<string> available_black_rock_moves(string piece, Cell rock)
        {
            Cell king = blackPieces["bk"];
            HashSet<string> availableMovements = new HashSet<string>();
        ...
        public static HashSet<string> available_black_queen_moves(string piece, Cell queen)
        {
            Cell king = blackPieces["bk"];
            HashSet<string> availableMovements = new HashSet<string>();
            foreach (var rockMovement in rockMovements)
        ...
        public static HashSet<string> available_white_moves()
        {
            HashSet<string> moves = new HashSet<string>();
        ...
        public static HashSet<string> available_white_king_moves()
        {
            ...
            foreach (var kingMovement in kingMovements)
            {
                ...
                if (nCell == "" || Regex.Match(nCell, @"^b").Success)
                {
                    ...
                    {
                        availableMovements.Add($"wk,{king.y + kingMovement.y},{king.x + kingMovement.x}");
                    }
                }
            }
            tempCells = (string[,])cells.Clone();
            ...
            {
                availableMovements.Add($"wk,{king.y},{king.x - 2}");
            }
        ...
        public static HashSet<string> available_white_knight_moves(string piece, Cell knight)
        {
            Cell king = whitePieces["wk"];
            HashSet<string> availableMovements = new HashSet<string>();
            foreach (var knightMovement in knightMovements)
        ...
        public static HashSet<string> available_white_pawn_moves(string piece, Cell pawn)
        {
            Cell king = whitePieces["wk"];
            HashSet<string> availableMovements = new HashSet<string>();
            string nCell = valid_position(pawn.y - 1, pawn.x);
            if (nCell == "")
            {
                ...
                if (!white_king_attacked(new Cell(king.y, king.x))) availableMovements.Add($"{piece},{pawn.y - 1},{pawn.x}");
            }
            if (pawn.y == 6)
            {
                nCell = valid_position(pawn.y - 2, pawn.x);
                if (nCell == "")
                {
                    ...
                    if (!white_king_attacked(new Cell(king.y, king.x))) availableMovements.Add($"{piece},{pawn.y - 2},{pawn.x}");
                }
            }
            nCell = valid_position(pawn.y - 1, pawn.x + 1);
            if (Regex.Match(nCell, @"^b").Success)
            {
                ...
                if (!white_king_attacked(new Cell(king.y, king.x))) availableMovements.Add($"{piece},{pawn.y - 1},{pawn.x + 1}");
            }
            nCell = valid_position(pawn.y - 1, pawn.x - 1);
            if (Regex.Match(nCell, @"^w").Success)
            {
            ...
            if (last_movement == $"bp,{pawn.y - 2},{pawn.x + 1},{pawn.y},{pawn.x + 1}")
            {
                tempCells = (string[,])cells.Clone();
                tempCells[pawn.y, pawn.x] = "";
                tempCells[pawn.y, pawn.x + 1] = "";
        ...
        public static HashSet<string> available_white_bishop_moves(string piece, Cell bishop)
        {
            Cell king = whitePieces["wk"];
            HashSet<string> availableMovements = new HashSet<string>();
            foreach (var bishopMovement in bishopMovements)
        ...
        public static HashSet<string> available_white_rock_moves(string piece, Cell rock)
        {
            Cell king = whitePieces["wk"];
            HashSet<string> availableMovements = new HashSet<string>();
            foreach (var rockMovement in rockMovements)
        ...
        public static HashSet<string> available_white_queen_moves(string piece, Cell queen)
        {
            Cell king = whitePieces["wk"];
            HashSet<string> availableMovements = new HashSet<string>();
            foreach (var rockMovement in rockMovements)
        ...
        public static bool attacked_by_black_pawn(int y, int x)
        {
            if (valid_temp_position(y - 1, x + 1) == "bp" || valid_temp_position(y - 1, x - 1) == "bp") return true;
            return false;
        }

        public static bool attacked_by_black_knight(int y, int x)
        {
        ...
        public static bool attacked_by_black_king(int y, int x)
        {
        ...
        public static bool attacked_by_black_in_diagonals(int y, int x)
        {
        ...
        public static bool attacked_by_black_in_rowcolumns(int y, int x)
        {
        ...
        public static bool attacked_by_white_pawn(int y, int x)
        {
            if (valid_temp_position(y + 1, x + 1) == "wp" || valid_temp_position(y + 1, x - 1) == "wp") return true;
            return false;
        }

        public static bool attacked_by_white_knight(int y, int x)
        {
        ...
        public static bool attacked_by_white_king(int y, int x)
        {
        ...
        public static bool attacked_by_white_in_diagonals(int y, int x)
        {
            List<Dictionary<string, int>> diagonals = new List<Dictionary<string, int>>();
            diagonals.Add(new Dictionary<string, int> { { "x", 1 }, { "y", 1 } });
            diagonals.Add(new Dictionary<string, int> { { "x", 1 }, { "y", -1 } });
        ...
        public static bool attacked_by_white_in_rowcolumns(int y, int x)
        {
            List<Dictionary<string, int>> rowcolumns = new List<Dictionary<string, int>>();
            rowcolumns.Add(new Dictionary<string, int> { { "x", 0 }, { "y", 1 } });
            rowcolumns.Add(new Dictionary<string, int> { { "x", 0 }, { "y", -1 } });
        ...
        public static string valid_temp_position(int y, int x)
        {
            try
            {
        ...
        public static string valid_position(int y, int x)
        {
            try
            {
                return cells[y, x];
        ...
        public static void Main(String[] args)
        {
            setInitialBoard();
            tempCells = (string[,])cells.Clone();
            List<string> black_movements =  available_black_moves().ToList<string>();
            Console.WriteLine(String.Join("\n", black_movements));
            Console.WriteLine($"Total movements: {black_movements.Count()}");
            List<string> white_movements = available_white_moves().ToList<string>();
            Console.WriteLine(String.Join("\n", white_movements));
